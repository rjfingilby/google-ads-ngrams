...
file_name = "Data/Product feed - N-Gram Report.csv"       #The name of the file to run this script on. By default, looks for this file in a "Data" folder in the same place as this script 
...
save_location = "Outputs/"                                #Where you want to save the file. By default, creates and folder in the same place as this script
...
ngrams_list = generate_ngrams(df[title_column_name], 3)
ngrams_df = build_ngrams_df(df, title_column_name,ngrams_list)
ngrams_df.head()
...
writer = pd.ExcelWriter(f"{save_location}{file_label} - N-Grams.xlsx", engine='xlsxwriter')
start_time = time.time()
